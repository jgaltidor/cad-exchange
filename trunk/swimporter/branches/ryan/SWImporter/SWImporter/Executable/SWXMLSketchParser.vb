...
Public Class SWXMLSketchParser
	...
	Public Sub createEntities()
		For Each node As XPathNavigator In getEntityNodes()
			createEntity(node)
            'Dim sketch As ISketch = swModel.GetActiveSketch2()
            'If(Not sketch.MergePoints(1.001))
	...
	Private Function createArc(ByRef node As XPathNavigator) As SketchSegment
        ...
        While childIterator.MoveNext()
            ...
            If childIterator.Current.Name.Equals("Radius")
	...
	Private Sub createConstraint(ByRef node As XPathNavigator)
		...
		swModel.SketchAddConstraints(getConstraintType(typ))
		swModel.ClearSelection2(True) 'Clear selection
	End Sub

    Private Function getConstraintType(ByRef type As String) As String
        Select Case type
            Case "swConstraintType_COINCIDENT"
                Return "sgCOINCIDENT"
            Case "swConstraintType_HORIZONTAL"
                Return "sgHORIZONTAL2D"
            ...
            Case "swConstraintType_DIAMETER"
                Return "sgDIAMETER"
            Case "swConstraintType_EQUALSIZE"
                Return "sgSAMELENGTH"
            Case Else
                Console.Error.WriteLine("Unknown constraint type: " & type)
        End Select
    End Function
	...
	Private Sub createDimension(ByRef node As XPathNavigator)
		...
		'Dim c As Double() = GetCoords(firstEnt) 'This should get coordinates of center?
